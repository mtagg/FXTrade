...
op = opy.APIv20(environment='practice',
                    access_token=CONFIG.access_token, timeout=3)        
...
def inTrade(dGain, transID):
     ...
     openP = (op.positions.get_open_positions(
                         account_id=CONFIG.account_id)).as_dict()
     ...
     #Fetch info about last trade 
          ...
          try:
               tran = (op.trades.get_trade_details(
                                   account_id=CONFIG.account_id, 
                                        trade_id=transID)).as_dict()                  
...
def writeTransaction(buySold, bid, ask, bet):
     ...
     try:
          ...
          with open(fileName, 'a') as FILE:
               ...
               FILE.write('\nAction: ' + buySold +'\n')
               FILE.write('Bet: '+ bet +'\n')
               FILE.write('Quantity: $' + str(CONFIG.LOT_SIZE) +'\n')
               FILE.write('Bid Price: $'+ str(bid) +'\n')
               FILE.write('Ask Price: $'+ str(ask) +'\n')
               FILE.write('\n\n')
